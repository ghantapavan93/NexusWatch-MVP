...
import { hasSourceDocument } from "@/lib/thresholdImpact";
  ...
  return (
    <div className="premium-card p-5">
      <div className="flex items-center justify-between">
        ...
        <Link href="/review" className="text-xs font-bold text-blue-700 hover:text-blue-800">
          View queue
        </Link>
      </div>
      <div className="mt-4 space-y-3">
        {invoices.slice(0, 5).map((invoice) => (
          <Link
            ...
            className="flex items-center justify-between gap-4 rounded-xl border border-slate-200 bg-white p-3 transition hover:bg-blue-50/40"
          >
            <div>
              <div className="text-sm font-medium text-slate-900">{invoice.invoiceNumber}</div>
              <div className="text-xs text-slate-500">
                {invoice.customerName || "Missing customer"} / {stateLabel(invoice.shipToState)} / {formatCurrency(invoice.taxableAmount)}
              </div>
              <div className="mt-1 text-xs text-slate-500">
                {hasSourceDocument(invoice) ? "Source document linked" : "No source document linked"}
              </div>
            </div>
        ...
        {invoices.length === 0 ? (
          <div className="rounded-xl border border-slate-100 bg-white p-4 text-sm text-slate-500">
            No invoices currently need review.
          </div>
        ) : null}
      </div>
    </div>
